<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="p-6">
        <label class="block text-sm font-medium text-gray-700 mb-3">
            Status Posyandu <span class="text-red-500">*</span>
        </label>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-[#000878] transition group">
                <input type="radio" 
                    name="status" 
                    value="Aktif" 
                    {{ old('status', $posyandu->status) == 'Aktif' ? 'checked' : '' }}
                    class="sr-only peer" 
                    required>
                <div class="flex items-center space-x-3 flex-1">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-gray-600">Posyandu beroperasi normal</div>
                    </div>
                </div>
            ...
            <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-[#000878] transition group">
                <input type="radio" 
                    name="status" 
                    value="Nonaktif" 
                    {{ old('status', $posyandu->status) == 'Nonaktif' ? 'checked' : '' }}
                    class="sr-only peer">
                <div class="flex items-center space-x-3 flex-1">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-gray-600">Posyandu tidak beroperasi</div>
                    </div>
                </div>
                <div class="absolute inset-0 border-2 border-[#000878] rounded-lg opacity-0 peer-checked:opacity-100 transition"></div>
            </label>
