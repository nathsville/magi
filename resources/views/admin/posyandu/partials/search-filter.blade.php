<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
    <form method="GET" action="{{ route('admin.posyandu') }}" id="filterForm" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="md:col-span-2 relative">
                ...
                <input type="text" 
                    ...
                    id="searchInput"
                    value="{{ $searchValue }}"
                    placeholder="Cari nama posyandu, kelurahan..." 
                    autocomplete="off"
                ...
                <div id="suggestionDropdown" 
                    class="absolute z-50 w-full mt-1 bg-white rounded-lg shadow-lg border border-gray-200 max-h-96 overflow-y-auto hidden">
            ...
            <div>
                <select name="puskesmas" 
                    ...
                    @foreach($puskesmasList as $pusk)
                        <option value="{{ $pusk->id_puskesmas }}" {{ $puskesmasValue == $pusk->id_puskesmas ? 'selected' : '' }}>
                            {{ $pusk->nama_puskesmas }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-600">
                Menampilkan {{ $totalCount }} dari {{ $totalAll }} posyandu
            </div>
