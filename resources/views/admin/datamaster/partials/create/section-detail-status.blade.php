    ...
    <div class="p-6 space-y-5">
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3" 
                placeholder="Tambahkan keterangan detail mengenai data ini..."
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#000878] focus:border-[#000878] resize-none">{{ old('deskripsi') }}</textarea>
            @error('deskripsi')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-[#000878] transition group">
                    <input type="radio" name="status" value="Aktif" {{ old('status', 'Aktif') == 'Aktif' ? 'checked' : '' }} class="sr-only peer">
                ...
                {{-- Radio Nonaktif --}}
                <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-[#000878] transition group">
                    <input type="radio" name="status" value="Nonaktif" {{ old('status') == 'Nonaktif' ? 'checked' : '' }} class="sr-only peer">
                    <div class="flex items-center space-x-3 flex-1">
